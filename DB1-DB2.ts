 ...
 */
export async function syncFilteredBooks(): Promise<void> {
  ...
  try {
    ...
    if (!books || books.length === 0) {
      console.log(`[${new Date().toISOString()}] No books to sync.`);
      return;
    }
    ...
    if (newBooks.length === 0) {
      console.log(`[${new Date().toISOString()}] No new books to insert.`);
      return;
    }
    ...
    console.log(`[${new Date().toISOString()}] ✅ Synced ${newBooks.length} books to DB2.`);
  } catch (err) {
    console.error(`[${new Date().toISOString()}] ❌ syncFilteredBooks failed:`, err);
    throw err; // Important for Deno.cron retries
  }
}

// === Allow direct execution when running locally ===
if (import.meta.main) {
